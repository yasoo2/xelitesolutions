...
| 038-LANGUAGE | جعل واجهة Joe مصدر لغة الرد | **منفذ محلياً؛ غير مدفوع** | `replyLanguageCode` مركزي، وتمرير `language` من `web/src/pages/Joe.tsx` إلى `/run/start` واستخدامه في AgentLoop وReactProjectTool؛ regression en/ar/fr وfallback script خضراء. أضيفت حالة Unicode حقيقية (`\u0750\u08A0\uFB50`) تثبت widened Arabic fallback. |
| 038-VISIBILITY | صدق Browser Self-QA ومرئية session الصحيحة | **منفذ محلياً؛ غير مدفوع** | نُقلت 3315022d انتقائياً: `CommandComposer` يرسل `browser:${sessionId}` كـfallback لكل run؛ `ReactProjectTool` ينتظر `context.browserSessionId`، يسجل `session/watching/liveSessions`، ويفرض `audit.visible=false` عند غياب watcher. regressions دائمة في `command-composer-browser-session` و`panel-watches` و`panel-shows`. |
| PR82-038 | قرار Claude 038 وAmendments A–C | **مُقرّ ومطبق محلياً عبر merges عادية؛ gates خضراء؛ language-default batch staged؛ push محجوب بterminal ref غير منشور** | acknowledgment منشور على PR #82؛ نُفّذ merge Claude `1442fc11` ثم `0a07ea6e` إلى local `main` في `d3c9ed5f`، ثم commit القياسات في `ea676281`، وبعدها normal merge لـ`origin/main=da586c92` في `f8ba2ca8` بلا rebase/reset/force. أُصلح ReactProjectTool على `replyLanguageCode`، ثم أزيلت خمسة defaults عربية من clarify/ImageStudio/ProjectRepair/GitHub report، مع CentralAnswer negative contract وUnicode fallback regression. focused language = 5 suites / 109 tests / exit 0؛ static API/Web TSC/build وdiff/control-byte = 0؛ full gate language batch = 21/21 batches، 248 suites، 4067 tests، 0 failures، كل exits = 0، process exit = 0، `GATE:PASS`. ما زال commit language ثم دمج terminal `f47c9166` عندما يصبح ref قابلاً للجلب، ثم final normal push وfresh isolated Joe rerun مطلوباً. |
| 038-ROUND10-11 | نقل عقد request-shape و58ff77b إلى main | **متحقق محلياً؛ full gate أخضر؛ language-default batch staged** | `derivedColumns` يفرق recording verb عن `fields/columns` المعلنة، question columns تصبح Yes/No أو نعم/لا مع statusField/doneValue، و`buildIntent` يحمل invented-verb مع possessive-object guard. أضيفت Oxford-comma وArabic attached-و regressions؛ focused language = 5 suites / 109 tests / PASS؛ full gate = 21/21 batches، 248 suites، 4067 tests، كل exits = 0، `GATE:PASS`. |
| 038-B7-1FD | صدق الطرفية والـrun من أول رسالة (`b7c7e341` + `1fd13b45`) | **منفذ محلياً عبر merge؛ language gate أخضر؛ terminal Arabic patch محجوب** | أضيف server fallback `browser:${sessionId}` في `/run/start`، وtool-aware `build_started` في Joe، و`explicitPanel` في JoeIDELayout؛ regression preview/tab behavior صار سلوكياً؛ static gates وfull gate language batch = 21/21 batches، 248 suites، 4067 tests، كل exits = 0، `GATE:PASS`. إصلاح Claude العربي الجديد `f47c9166` لم يُدمج لأن ref `claude/arabic-in-the-terminal` غير منشور؛ raw `5388588551`. |
| 038-QUALITY-REPAIR | إصلاح `flat_hierarchy` لا `type_scale_drift` (`0a07ea6e`) | **مطبق محلياً عبر merge؛ focused/full أخضران؛ ينتظر اللغة والطرفية ثم push** | Claude أثبت أن audit يخرج `flat_hierarchy` بلا repair؛ merge أضاف arithmetic escalation في `ui-repair.ts` واختبار `a-heading-that-is-actually-a-heading.test.ts`. full gate language batch = 21/21 batches، 248 suites، 4067 tests، `GATE:PASS`. |
| 038-PREVIEW-BEHAVIOR | فتح Preview بعد الجاهزية الكاملة فقط | **مطبق محلياً؛ focused/full أخضران؛ ينتظر commit/push ثم eye/live proof** | أضيف `web/src/lib/preview-routing.ts` وارتبط بـ`Joe.tsx`: complete internal `preview_ready` يفتح Preview، partial/external/unrelated لا يخطف Logs؛ regression سلوكي؛ full gate الحالي `GATE:PASS` (248 suites / 4067 tests). |
| 038-TEST-LIFETIME | منع provider/timer leaks في unit planner | **مطبق محلياً؛ focused/full أخضران؛ ينتظر commit/push** | raw `5388259765` أثبت 9/9 مع async provider leak وexit 1؛ بعد القرار أضيف mock provider deterministic ويمسح `PlanningEngine` race timers في finally، كما نظّف api-project test deadlines. a-door detect/no-detect كلاهما exit 0 بلا late/open-handle warning؛ full gate language batch = `GATE:PASS` (248 suites / 4067 tests). |
| 038-ACCEPTANCE-ALIGNMENT | dynamic acceptance IDs وrequest-shaped API/React schema | **منفذ محلياً؛ focused/full أخضران؛ ينتظر commit/push ثم live proof** | raw `5388347272` أثبت throw على `column:*`؛ raw `5388399983` أثبت أن إزالة throw كشفت stock schema mismatch؛ diagnosis `5388412439` وقرار Claude `5388428227`؛ الإصلاح يقبل `column:<safe-key>` فقط، يترك unknown IDs loud، يجعل request fields authoritative للـprimary linked table، ويزيل English Oxford-comma conjunction دون لمس Arabic attached `و`. clinic linked handoff وdirect reconciliation/Arabic regressions خضراء؛ full gate language batch `GATE:PASS` (248 suites / 4067 tests). |
...
`POS-2026-08-23-038-LANGUAGE-STAGED`: merge commit `f8ba2ca8` يضم `origin/main=da586c92` بدمج عادي فوق `ea676281`، واللغة الموسعة staged فوقه. focused language = 5 suites / 109 tests / process exit 0، delivered-store Unicode real execution = 17/17، static API/Web TSC/build + cached/worktree diff checks + control-byte scan = 0؛ full gate language batch = 21/21 batches، 248 suites، 4067 tests، 0 failures، كل batch exits = 0، `GATE:PASS`، process exit = 0. لا يمكن إكمال final push بعد لأن terminal commit `f47c9166` ليس له remote ref قابل للجلب؛ raw `5388588551` وdiagnosis `5388589304`. يجب commit language، دمج terminal عند نشره، إعادة gate combined، ثم normal push إلى `main` وfresh Eye/Clinic/Prompt 01 live proof. لا generated/package/lock/WeatherGo files في الدفعة.
