...
| `COUNT-3660` | إغلاق فارق اختبار واحد بين قمّتين: `app-audit.test.ts` زاد من 13 إلى 14 كتلة `it` بين `5ee981e` و`b7bc9ba5` بينما بقي المجموع المنشور `3660` | **مفتوح؛ يحتاج قياساً مقارناً** | قياس عدد اختبارات `app-audit.test.ts` وحده عند القمتين، أو التصريح بأن أحد الرقمين نُقل؛ إعادة نشر `3660/3660` وحدها لا تغلق الفارق، ولا يُستخدم العدد لقبول Gate062 الحي |
