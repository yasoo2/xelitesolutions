...
| PR82-038 | قرار Claude 038 وAmendments A–C | **مُقرّ ومطبق انتقائياً؛ full gate أخضر وينتظر المصالحة والدفع** | acknowledgment منشور على PR #82؛ لم يُدمج فرع Claude wholesale. focused بعد port = 9 suites / 127 tests / PASS بلا open-handle؛ API TSC/build وWeb TSC/build و`git diff --check` = 0؛ full gate المباشر بعد visibility = 20/20 batches، 233 suites، 3811 tests، `GATE_TSC_EXIT:0` و`GATE_WEB_TSC_EXIT:0` و`GATE_API_BUILD_EXIT:0` و`GATE_WEB_BUILD_EXIT:0` و`GATE_JEST_EXIT:0` و`GATE:PASS`، process exit = 0. يلزم cached diff review، safe reconciliation إلى `origin/main`، commit/push main، ثم fresh isolated Joe rerun. |
| 038-ROUND10-11 | نقل عقد request-shape و58ff77b إلى main | **متحقق محلياً؛ غير مدفوع** | `derivedColumns` يفرق recording verb عن `fields/columns` المعلنة، question columns تصبح Yes/No أو نعم/لا مع statusField/doneValue، و`buildIntent` يحمل invented-verb مع possessive-object guard. Regression دائم للتtracking/WeatherGo/negative boundaries وbare UI noun؛ focused بعد visibility = 9 suites / 127 tests / PASS؛ full gate = 20/20 batches، 233 suites، 3811 tests، exit 0 و`GATE:PASS`. |
| 038-B7-1FD | صدق الطرفية والـrun من أول رسالة (`b7c7e341` + `1fd13b45`) | **منفذ محلياً؛ غير مدفوع** | وفق قرار Claude 5388063815، أضيف server fallback `browser:${sessionId}` في `/run/start`، وtool-aware `build_started` في Joe، و`explicitPanel` في JoeIDELayout كي يغلب `panel_focus(reason)` heuristic `file_stream` حتى يختار المستخدم أو يصرّح الخادم بلوحة أخرى. أضيفت regressions في `run-start-browser-fallback.test.ts` و`live-code-stream.test.ts`؛ focused = 11 suites/143 tests/PASS؛ static API/Web TSC/build و`git diff --check` = 0؛ full gate = 20/20 batches، 234 suites، 3815 tests، كل exits = 0، process exit = 0، `GATE:PASS`. يلزم commit/push، ثم eye/live proof. |
...
`POS-2026-08-23-038-LOCAL`: port naming + request-shape + 3315022d + approved b7c7e341/1fd13b45 مكتمل محلياً؛ focused = 11 suites/143 tests/PASS؛ static API/Web TSC/build وdiff-check = 0؛ full gate بعد b7/1fd = 20/20 batches، 234 suites، 3815 tests، كل exits = 0، process exit = 0، `GATE:PASS`. التغييرات ما زالت محلية غير مدفوعة على `main`، ولا generated/package/lock/WeatherGo files؛ يلزم cached review، safe reconciliation/commit/push ثم eye/clinic/Prompt 01 live proof.
