...
| 057 | تمرير هوية المشروع القانونية إلى `PhaseExecutor` قبل تنفيذ `verificationTask` حتى لا يعود التحقق إلى جذر مساحة العمل | **منفذ محلياً؛ regression أخضر؛ TSC=0؛ Jest الكاملة 22/22 دفعة؛ الدفع والتحقق الحي قيد التنفيذ** | استدعاء `inheritRuntimeProjectArguments(...)` قبل تكييف حجة التحقق؛ regression دائم يثبت إعادة ربط `code_reviewer` إلى `projectRootRuntimeBound`؛ تنظيف مساحة المشاريع قبل الجولة 19؛ دفع إلى `main` ثم تشغيل البرومبت القانوني حياً |
