...
## قرار Claude 038 — تنفيذ انتقائي قيد الاختبار — 2026-08-23
| المعرّف | المهمة | الحالة | الدليل ومعيار الإغلاق |
|---|---|---|---|
| 038-RECEIPT | حفظ provenance لـCodeReviewer وعدم إسقاط findings | **منفذ محلياً؛ غير مدفوع** | `CodeReviewerTool` يميز deterministic findings الموثقة عن LLM proposals الموسومة `unverified`، و`AgentLoopService` يسجل `reviewer_findings_lost` عند وجود critical aggregate بلا issue objects. اختبارات `code-reviewer-workspace` و`run-evidence` خضراء. |
| 038-SHAPE | توحيد build intent وقراءة الأعمدة/الموضوع من شكل الطلب | **منفذ محلياً؛ غير مدفوع** | أضيف `buildIntent.ts` مشترك، وport إلى `PlanningEngine` و`IntentParser`، وقراءة `derivedColumns`/`recordedSubject` دون domain catalogue، مع regressions unseen noun/feature verbs. |
| 038-NAME | ضبط حدود الاسم والـowns subject | **منفذ محلياً؛ غير مدفوع** | `page-head.ts` يرفض marker داخل `باسمه` ويستخلص `عندي/أملك/I run`؛ نُقلت حالات Claude 2507d332 ثنائية الاتجاه (`للقهوة`/`للمأكولات`/`لبيع` موجبة وbare `ل` سالبة) إلى `a-name-ends-where-the-sentence-resumes.test.ts`. delivery-details + name-boundary خضرا. |
| 038-LANGUAGE | جعل واجهة Joe مصدر لغة الرد | **مدفوع إلى origin/main ضمن 4688c41c؛ live proof pending** | `replyLanguageCode` مركزي، وتمرير `language` من `web/src/pages/Joe.tsx` إلى `/run/start` واستخدامه في AgentLoop وReactProjectTool؛ regression en/ar/fr وfallback script خضراء. أضيفت حالة Unicode حقيقية (`\u0750\u08A0\uFB50`) تثبت widened Arabic fallback. |
| 038-VISIBILITY | صدق Browser Self-QA ومرئية session الصحيحة | **مدفوع إلى origin/main ضمن 4688c41c؛ live proof pending** | نُقلت 3315022d انتقائياً: `CommandComposer` يرسل `browser:${sessionId}` كـfallback لكل run؛ `ReactProjectTool` ينتظر `context.browserSessionId`، يسجل `session/watching/liveSessions`، ويفرض `audit.visible=false` عند غياب watcher. regressions دائمة في `command-composer-browser-session` و`panel-watches` و`panel-shows`؛ combined full gate أخضر (249/4089). |
| PR82-038 | قرار Claude 038 وAmendments A–C | **4688c41c مدفوع إلى origin/main؛ هذا ledger correction pending كـnormal descendant؛ combined gates خضراء؛ live proof جارٍ، Prompt 01 ما زال OPEN** | acknowledgment منشور على PR #82؛ نُفّذ merge Claude `1442fc11` ثم `0a07ea6e` إلى local `main` في `d3c9ed5f`، ثم commit القياسات `ea676281` وnormal merge لـ`origin/main=da586c92` في `f8ba2ca8` بلا rebase/reset/force، ثم language commit `e408f12c` وnormal merge terminal `f47c9166` في `fa397045`. أُصلح ReactProjectTool على `replyLanguageCode`، ثم أزيلت خمسة defaults عربية من clarify/ImageStudio/ProjectRepair/GitHub report، مع CentralAnswer negative contract وUnicode fallback regression، ودمجت Arabic terminal patch المستقل. combined focused = 12 suites / 342 tests / exit 0؛ static API/Web TSC/build وdiff/control-byte = 0؛ combined full gate = 21/21 batches، 249 suites، 4089 tests، 0 failures، كل exits = 0، process exit = 0، `GATE:PASS`. ما زال final listener ثم normal push إلى `origin/main` وfresh isolated Joe rerun مطلوباً. |
| 038-ROUND10-11 | نقل عقد request-shape و58ff77b إلى main | **مدفوع إلى origin/main ضمن 4688c41c؛ live proof pending** | `derivedColumns` يفرق recording verb عن `fields/columns` المعلنة، question columns تصبح Yes/No أو نعم/لا مع statusField/doneValue، و`buildIntent` يحمل invented-verb مع possessive-object guard. أضيفت Oxford-comma وArabic attached-و regressions؛ combined focused = 12 suites / 342 tests / PASS؛ combined full gate = 21/21 batches، 249 suites، 4089 tests، كل exits = 0، `GATE:PASS`. |
| 038-B7-1FD | صدق الطرفية والـrun من أول رسالة (`b7c7e341` + `1fd13b45`) | **مدفوع إلى origin/main ضمن 4688c41c؛ combined gate أخضر؛ ينتظر eye/live proof** | أضيف server fallback `browser:${sessionId}` في `/run/start`، وtool-aware `build_started` في Joe، و`explicitPanel` في JoeIDELayout؛ regression preview/tab behavior صار سلوكياً. ثم دُمج terminal Arabic patch `f47c9166812061b89e5ac536b2109fa0a7677ad6` بدمج عادي في `fa397045`; terminal regression suite والـcombined focused/static/full كلها خضراء؛ full = 249 suites / 4089 tests، `GATE:PASS`. |
| 038-QUALITY-REPAIR | إصلاح `flat_hierarchy` لا `type_scale_drift` (`0a07ea6e`) | **مدفوع إلى origin/main ضمن 4688c41c؛ combined focused/full أخضران؛ live proof pending** | Claude أثبت أن audit يخرج `flat_hierarchy` بلا repair؛ merge أضاف arithmetic escalation في `ui-repair.ts` واختبار `a-heading-that-is-actually-a-heading.test.ts`. combined full gate = 21/21 batches، 249 suites، 4089 tests، `GATE:PASS`. |
| 038-PREVIEW-BEHAVIOR | فتح Preview بعد الجاهزية الكاملة فقط | **مدفوع إلى origin/main ضمن 4688c41c؛ focused/full أخضران؛ ينتظر eye/live proof** | أضيف `web/src/lib/preview-routing.ts` وارتبط بـ`Joe.tsx`: complete internal `preview_ready` يفتح Preview، partial/external/unrelated لا يخطف Logs؛ regression سلوكي؛ combined full gate `GATE:PASS` (249 suites / 4089 tests). |
| 038-TEST-LIFETIME | منع provider/timer leaks في unit planner | **مدفوع إلى origin/main ضمن 4688c41c؛ focused/full أخضران** | raw `5388259765` أثبت 9/9 مع async provider leak وexit 1؛ بعد القرار أضيف mock provider deterministic ويمسح `PlanningEngine` race timers في finally، كما نظّف api-project test deadlines. a-door detect/no-detect كلاهما exit 0 بلا late/open-handle warning؛ combined full gate = `GATE:PASS` (249 suites / 4089 tests). |
| 038-ACCEPTANCE-ALIGNMENT | dynamic acceptance IDs وrequest-shaped API/React schema | **مدفوع إلى origin/main ضمن 4688c41c؛ combined full أخضر؛ ينتظر live proof** | raw `5388347272` أثبت throw على `column:*`؛ raw `5388399983` أثبت أن إزالة throw كشفت stock schema mismatch؛ diagnosis `5388412439` وقرار Claude `5388428227`؛ الإصلاح يقبل `column:<safe-key>` فقط، يترك unknown IDs loud، يجعل request fields authoritative للـprimary linked table، ويزيل English Oxford-comma conjunction دون لمس Arabic attached `و`. clinic linked handoff وdirect reconciliation/Arabic regressions خضراء؛ combined full gate `GATE:PASS` (249 suites / 4089 tests). |
