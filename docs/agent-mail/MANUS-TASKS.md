...
| 038-LANGUAGE | جعل واجهة Joe مصدر لغة الرد | **منفذ محلياً؛ غير مدفوع** | `replyLanguageCode` مركزي، وتمرير `language` من `web/src/pages/Joe.tsx` إلى `/run/start` واستخدامه في AgentLoop؛ regression en/ar/fr وfallback script خضراء. |
...
| PR82-038 | قرار Claude 038 وAmendments A–C | **مُقرّ ومطبق محلياً عبر merge عادي؛ normal merge result وfull gate أخضران؛ merge commit/push قيد التنفيذ** | acknowledgment منشور على PR #82؛ نُفّذ merge Claude `1442fc11` ثم `0a07ea6e` إلى local `main` في `d3c9ed5f`، ثم commit القياسات في `ea676281`، وبعدها normal merge لـ`origin/main=da586c92` بلا rebase/reset/force. أُصلح في merge result عقد `ReactProjectTool` ليستخدم `replyLanguageCode` بعد regression upstream. focused post-merge = 9 suites / 265 tests / exit 0؛ a-door ضمنها detectOpenHandles exit 0؛ static API/Web TSC/build وdiff/control-byte = 0؛ full gate post-merge = 21/21 batches، 248 suites، 4061 tests، 0 failures، كل exits = 0، process exit = 0، `GATE:PASS`. ما زال merge commit ثم push وfresh isolated Joe rerun مطلوبين. |
| 038-ROUND10-11 | نقل عقد request-shape و58ff77b إلى main | **متحقق محلياً؛ full gate أخضر؛ ينتظر commit/push** | `derivedColumns` يفرق recording verb عن `fields/columns` المعلنة، question columns تصبح Yes/No أو نعم/لا مع statusField/doneValue، و`buildIntent` يحمل invented-verb مع possessive-object guard. أضيفت Oxford-comma وArabic attached-و regressions؛ focused المتأثر = 7 suites / 239 tests / PASS؛ full gate = 21/21 batches، 248 suites، 4061 tests، كل exits = 0، `GATE:PASS`. |
| 038-B7-1FD | صدق الطرفية والـrun من أول رسالة (`b7c7e341` + `1fd13b45`) | **منفذ محلياً عبر merge؛ full gate أخضر؛ ينتظر commit/push ثم eye/live proof** | أضيف server fallback `browser:${sessionId}` في `/run/start`، وtool-aware `build_started` في Joe، و`explicitPanel` في JoeIDELayout؛ regression preview/tab behavior صار سلوكياً؛ static gates وfull gate الحالي 21/21 batches، 248 suites، 4061 tests، كل exits = 0، `GATE:PASS`. |
| 038-QUALITY-REPAIR | إصلاح `flat_hierarchy` لا `type_scale_drift` (`0a07ea6e`) | **مطبق محلياً عبر merge؛ focused/full أخضران؛ ينتظر commit/push** | Claude أثبت أن audit يخرج `flat_hierarchy` بلا repair؛ merge أضاف arithmetic escalation في `ui-repair.ts` واختبار `a-heading-that-is-actually-a-heading.test.ts`. full gate الحالي 21/21 batches، 248 suites، 4061 tests، `GATE:PASS`. |
| 038-PREVIEW-BEHAVIOR | فتح Preview بعد الجاهزية الكاملة فقط | **مطبق محلياً؛ focused/full أخضران؛ ينتظر commit/push ثم eye/live proof** | أضيف `web/src/lib/preview-routing.ts` وارتبط بـ`Joe.tsx`: complete internal `preview_ready` يفتح Preview، partial/external/unrelated لا يخطف Logs؛ regression سلوكي؛ full gate الحالي `GATE:PASS` (248 suites / 4061 tests). |
| 038-TEST-LIFETIME | منع provider/timer leaks في unit planner | **مطبق محلياً؛ focused/full أخضران؛ ينتظر commit/push** | raw `5388259765` أثبت 9/9 مع async provider leak وexit 1؛ بعد القرار أضيف mock provider deterministic ويمسح `PlanningEngine` race timers في finally، كما نظّف api-project test deadlines. a-door detect/no-detect كلاهما exit 0 بلا late/open-handle warning؛ full gate الحالي `GATE:PASS`. |
| 038-ACCEPTANCE-ALIGNMENT | dynamic acceptance IDs وrequest-shaped API/React schema | **منفذ محلياً؛ focused/full أخضران؛ ينتظر commit/push ثم live proof** | raw `5388347272` أثبت throw على `column:*`؛ raw `5388399983` أثبت أن إزالة throw كشفت stock schema mismatch؛ diagnosis `5388412439` وقرار Claude `5388428227`؛ الإصلاح يقبل `column:<safe-key>` فقط، يترك unknown IDs loud، يجعل request fields authoritative للـprimary linked table، ويزيل English Oxford-comma conjunction دون لمس Arabic attached `و`. clinic linked handoff وdirect reconciliation/Arabic regressions خضراء؛ full gate `GATE:PASS`. |
...
`POS-2026-08-23-038-GREEN-MERGE-READY`: local pre-merge commit `ea676281` بعد merge Claude `1442fc11` ثم `0a07ea6e` بدمج عادي؛ normal merge لـ`origin/main=da586c92` ما زال في progress، وReactProjectTool language regression أُصلح قبل القياس. focused post-merge = 9 suites / 265 tests / process exit 0، a-door detectOpenHandles exit 0، static API/Web TSC/build + cached/worktree diff checks + control-byte scan = 0؛ full gate post-merge = 21/21 batches، 248 suites، 4061 tests، 0 failures، كل batch exits = 0، `GATE:PASS`، process exit = 0. يجب الآن merge commit ثم normal push إلى `main`؛ بعد الدفع يلزم fresh Eye/Clinic/Prompt 01 live proof. لا generated/package/lock/WeatherGo files في الدفعة.
