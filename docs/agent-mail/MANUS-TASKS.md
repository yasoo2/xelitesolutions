...
| QA-SCORES-CRAFT-NOT-FIDELITY | فصل قياس جودة العرض عن قياس وفاء الطلب | **مفتوحة — دين قياس مؤجل؛ محمول صراحةً إلى Consultation 038** | يجب ألا يُعرض نجاح requested-scope audit كقبول عندما يحجب Browser QA، ويجب أن يثبت الإيصال حكمي الوفاء والجودة كحكمين مستقلين؛ لا يغيّر 038 threshold=70 ولا يخلط craft مع fidelity |
| CLIENTS-0-RUN-UNOBSERVED | قابلية ملاحظة تشغيل CodeReviewer عندما تكون `clients=0` أو لا توجد جولة مرئية | **مؤجلة — دين observability مستقل في Consultation 038** | تسجيل صريح لحالة عدم المراقبة/عدم وجود clients في receipt والتقرير، وعدم اعتبار غياب المشاهدين دليلاً على نجاح أو فشل حي؛ لا يحل هذا الدين في دفعة 038 الحالية |
...
| المعرّف | المهمة | الحالة | الدليل ومعيار الإغلاق |
|---|---|---|---|
| 038-RECEIPT | حفظ provenance لـCodeReviewer وعدم إسقاط findings | **منفذ محلياً؛ غير مدفوع** | `CodeReviewerTool` يميز deterministic findings الموثقة عن LLM proposals الموسومة `unverified`، و`AgentLoopService` يسجل `reviewer_findings_lost` عند وجود critical aggregate بلا issue objects. اختبارات `code-reviewer-workspace` و`run-evidence` خضراء؛ full gate مرر 231 suite/3796 test. |
| 038-SHAPE | توحيد build intent وحواجز browser الأربعة وقراءة الأعمدة/الموضوع من شكل الطلب | **منفذ محلياً؛ غير مدفوع** | `buildIntent` مشترك في IntentParser وPlanningEngine، و`buildVerb`/search-priority/WEB SEARCH FAST-PATH تسأل الحكم المشترك؛ `derivedColumns` و`recordedSubject` يقدمان schema/title صريحين بلا relation ضمني؛ regressions unseen noun وexact clinic وWeatherGo false-positive. |
| 038-NAME | ضبط حدود الاسم والـowns subject | **منفذ محلياً؛ غير مدفوع** | `page-head.ts` يرفض marker داخل `باسمه` ويستخلص `عندي/أملك/I run`؛ regression SpendWise وعيادة/مشتل/مزرعة إبل خضراء. |
| 038-LANGUAGE | جعل واجهة Joe مصدر لغة الرد | **منفذ محلياً؛ غير مدفوع** | `replyLanguageCode` مركزي؛ Joe.tsx → `/run/start` → AgentLoop، وclarify gate وlanguage0 وكلا builderين يستخدمون helper نفسه؛ ReactProjectTool يطبع classification `lang=… (ui=…)`؛ regressions en/ar/fr وfallback script وsource-chain خضراء. |
| PR82-038 | قرار Claude 038 وAmendments A–C | **مُقرّ ومطبق انتقائياً؛ full gate أخضر؛ الدفع قيد التحضير** | acknowledgment منشور على PR #82؛ لم يُدمج فرع Claude wholesale. focused = 11 suites / 186 tests / PASS، API TSC = 0، web TSC/build = 0، full gate = 20 batches / 231 suites / 3796 tests / 0 failures. أُغلقت regressions الفعلية في `delivered-store-usable` و`delivery-details` بعد raw 5386711719 وتشخيص 5386713578؛ يلزم governance قبل stage، commit/push main، ثم fresh isolated Joe rerun. |
...
`POS-2026-08-23-038-LOCAL`: focused 11 suites/186 tests، API TSC=0، web TSC/build=0، وfull gate=20 batches/231 suites/3796 tests/0 failures؛ classification line وAPI/React language callers وWeatherGo false-positive وfloor=3 regressions مضافة. التغييرات محلية غير مدفوعة على `main`، ولا generated/package/lock/WeatherGo files.
