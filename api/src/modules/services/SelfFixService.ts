...
export class SelfFixService {
  static plan(ticket: RepairTicket): SelfFixPlan {
    ...
    const missingFileMatch = /missing_file:\s*([^\s]+)/i.exec(text);
    if (missingFileMatch) {
      const filename = missingFileMatch[1].replace(/['";]/g, '');
      return {
        type: 'self_fix_plan',
        allowed: true,
        reason: `Detected missing file: ${filename}. Attempting auto-creation.`,
        maxAttempts: 1,
        strategy: 'code_fix',
        suggestedTool: 'write_file',
        suggestedInput: {
          filename: filename,
          content: '// Auto-generated by SelfFixService to resolve missing file error\n',
        },
        safety: this.safety(),
        sourceTicket: ticket,
      };
