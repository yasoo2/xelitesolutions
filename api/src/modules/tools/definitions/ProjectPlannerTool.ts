...
export class ProjectPlannerTool implements ToolDefinition {
    ...
    async execute(input: { projectDescription: string; analysis?: any; evidence?: EngineeringEvidence }, context?: any) {
        ...
        try {
            ...
            const scopeRepairMode = context?.scopeRepairMode === true;
            const scopeRepairTargets = Array.isArray(context?.scopeRepairTargets)
                ? context.scopeRepairTargets.map((target: unknown) => String(target || '').trim()).filter(Boolean).slice(0, 12)
                : [];
            ...
            let scopeAssessment = this.assessPlanScope(plan, projectDescription, repairMode, scopeRepairMode, scopeRepairTargets);
            if (!scopeAssessment.ok && requestedImplementation) {
                ...
                while (!scopeAssessment.ok && scopeRecoveryAttempt < maxScopeRecoveryAttempts) {
                    ...
                    try {
                        ...
                        const recoveredAssessment = this.assessPlanScope(
                            recoveredValidatedPlan,
                            projectDescription,
                            repairMode,
                            scopeRepairMode,
                            scopeRepairTargets,
...
Include build, browser QA, visual QA, and self-healing verification tasks where relevant.`;
     ...
     */
    private createCompactRecoveryPlanningPrompt(
        projectDescription: string,
        failureReason: string,
        assessment?: { missingTargetNames?: string[]; message?: string },
        repairMode = false,
        scopeRepairMode = false,
        scopeRepairTargets: string[] = [],
        evidence?: EngineeringEvidence,
...
Schema: {"projectName":"quality-repair","projectVibe":"bounded quality-contract repair","totalPhases":1,"estimatedDuration":"short estimate","phases":[{"phaseNumber":1,"name":"Repair quality contract","description":"Add the evidenced truthful test command and deterministic test artifact without redesigning the project.","requirementsCovered":["R1"],"tasks":[{"task":"update the existing manifest with a truthful test script","tool":"ai_write_file","args":{"path":"package.json","description":"Preserve existing scripts and add a real local test command such as node --test only when supported by the inspected runtime and test file."},"priority":"high","realisticMinutes":10},{"task":"add a deterministic test for existing behavior","tool":"ai_write_file","args":{"path":"tests/quality-contract.test.js","description":"Create a small deterministic test that imports or exercises an existing project module; do not use a placeholder assertion."},"priority":"high","realisticMinutes":15}],"verificationTask":"Run the truthful test command, build, and one real start/readiness check.","deliverables":["updated manifest and real test artifact"],"estimatedTime":"25 minutes"}],"dependencies":[]}`;
        ...
        if (scopeRepairMode) {
            const repairTargets = scopeRepairTargets.length
                ? scopeRepairTargets
                : (assessment?.missingTargetNames || ['the missing named capabilities']);
            const targets = repairTargets
                .map((target, index) => `R${index + 1}: ${String(target).slice(0, 220)}`)
                .join(' | ');
            const ids = repairTargets.map((_target, index) => `R${index + 1}`);
            const phaseCount = Math.min(3, Math.max(1, Math.ceil(repairTargets.length / 2)));
            return `BOUNDED SCOPE-REPAIR — extend the existing runnable project only; do not redesign or rebuild it.

${ENGINEERING_CONSTITUTION}

Failure evidence: ${String(failureReason || '').slice(0, 1200)}

MISSING CAPABILITY LEDGER (implement every item, using the current workspace as evidence):
${targets}

Return ONLY one valid JSON object with 1-3 compact phases and no prose. Inspect the current workspace before choosing exact source paths. Use only executable file-level implementation tasks with safe workspace-relative paths. Modify the existing project in place; do not create a template, second project, unrelated foundation, or documentation-only phase. Each missing capability must have concrete source evidence and a local verification task. Re-run the existing build/tests and one real start/readiness check after the changes. For React/Vite/Next/Expo, that start/readiness check MUST use project_run with the evidenced project cwd, not shell_execute node on a TypeScript/JSX entrypoint. Use only test runners and browser dependencies proven by the inspected manifest. Use requirementsCovered: ${JSON.stringify(ids)} and keep totalPhases equal to the phases returned.
...
Repeat the same compact phase shape for at least ${scope.minPhases} phases and no more than 8 phases, changing paths, descriptions, and R-number coverage according to the coverage ledger. Each R-ID must advance a concrete file-level implementation task; verification tasks may be included but must not replace implementation coverage. Do not output anything before or after the JSON object.`;
    ...
    private assessPlanScope(
        ...
        if (scopeRepairMode) {
            ...
            if (phases.length < 1 || phases.length > 3) problems.push(`it has ${phases.length} phase(s), but scope repair allows 1-3`);
            if (implementationArtifacts < 1) problems.push('it has no non-document implementation artifact task');
            if (missingTargetNames.length) problems.push(`missing capability coverage: ${missingTargetNames.join(', ')}`);
            return {
                ok: problems.length === 0,
                message: problems.length ? `Bounded scope-repair plan is invalid: ${problems.join('; ')}.` : 'Bounded scope-repair scope is sufficient for controlled execution.',
