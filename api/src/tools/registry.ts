...
`;
            ...
            // 2. API (Backend)
            const hasProducts = features.includes('products') || type === 'ecommerce';
            
            files['apps/api/package.json'] = JSON.stringify({
                name: "api",
                version: "0.1.0",
                scripts: { "dev": "nodemon src/index.js", "build": "tsc", "start": "node dist/index.js" },
...
`;
            files['apps/api/src/index.js'] = apiIndex.trim();
            ...
            files['apps/web/index.html'] = `<!doctype html><html lang="en" dir="rtl"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>${projectName}</title></head><body><div id="root"></div><script type="module" src="/src/main.jsx"></script></body></html>`;
...
function App() {
  ...
  useEffect(() => {
    axios.get('/api').then(r => setStatus(r.data.status)).catch(() => setStatus('Offline'));
    ${hasProducts ? "axios.get('/api/products').then(r => setProducts(r.data)).catch(console.error);" : ""}
  }, []);

  return (
    <div className="min-h-screen bg-slate-900 text-white">
      <header className="p-4 border-b border-slate-800 flex justify-between items-center backdrop-blur bg-slate-900/80 sticky top-0 z-50">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-emerald-400 text-transparent bg-clip-text">${projectName}</h1>
        <div className="flex items-center gap-4">
            <span className="text-xs px-2 py-1 rounded bg-slate-800 border border-slate-700 text-slate-400">API: {status}</span>
            <button className="p-2 hover:bg-slate-800 rounded-full"><ShoppingBag /></button>
        </div>
      </header>
      
      <main className="max-w-7xl mx-auto p-6">
        <div className="text-center py-12">
            <h2 className="text-5xl font-extrabold mb-4">Welcome to ${projectName}</h2>
            <p className="text-slate-400 text-xl">Powered by Joe AI</p>
        </div>

        ${hasProducts ? `
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
            {products.map((p, i) => (
                <div key={i} className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700 hover:border-blue-500 transition-all hover:shadow-2xl">
                    <div className="h-48 bg-slate-700 flex items-center justify-center">
                        <img src={p.image || 'https://placehold.co/400x300'} alt={p.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="p-6">
                        <div className="text-xs font-bold text-blue-400 uppercase mb-2">{p.category || 'Item'}</div>
                        <h3 className="text-xl font-bold mb-2">{p.name}</h3>
                        <div className="flex justify-between items-center mt-4">
                            <span className="text-2xl font-bold text-emerald-400">\${p.price}</span>
                            <button className="bg-white text-slate-900 px-4 py-2 rounded-lg font-bold hover:bg-blue-50">Add</button>
                        </div>
                    </div>
                </div>
            ))}
...
export default App;
`;
            files['apps/web/src/App.jsx'] = webApp.trim();
