  ...
  if (providerKey.includes('gemini') || providerKey.includes('google')) {
    ...
    try {
      ...
      if (options?.provider === 'auto') {
        console.error('[LLM] Critical: Auto-mode fallback failed. Using terminal pollinations fallback.');
        ...
        return { name: 'echo', input: { text: text || 'Response from Joe (Emergency Fallback)' } };
      }
