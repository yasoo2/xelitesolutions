...
describe('the self-QA reaches the user in whatever language he asked in', () => {
    it('keeps the live status tied to the action actually under test', () => {
        const audit = fs.readFileSync(path.join(__dirname, '..', 'core', 'quality', 'app-audit.ts'), 'utf-8');
        const react = SRC();
        // Form semantics and state exploration are evidence generated by the
        // probe itself, not a cosmetic script in the chat panel.
        expect(audit).toContain('onProgress: opts?.onProgress');
        expect(react).toContain("where === 'forms'");
        expect(react).toContain("where === 'exploring'");
        expect(react).toContain("where.startsWith('state:')");
        expect(react).toContain("where === 'discovering'");
        expect(react).toContain('لن أتعامل مع الصفحة كقائمة نقرات محفوظة');
    });

    it('does not mistake custom empty-submit validation for a missing required attribute', () => {
        const behaviour = fs.readFileSync(path.join(__dirname, '..', 'core', 'quality', 'behaviour-audit.ts'), 'utf-8');
        expect(behaviour).toContain('formsEmptyRejected');
        expect(behaviour).toContain('emptyInvalid || emptyEffect');
        expect(behaviour).toContain('Number(metrics.formsWithoutValidation || 0) - Number(r.metrics.formsEmptyRejected || 0)');
    });
