...
async function run(e){e?.preventDefault();if(!validIp(ip)){setData(null);setError('');setIpError('Enter a valid IPv4 address, for example 8.8.8.8');return}setIpError('');setLoading(true);setError('');setData(null);try{setData(await externalApi.load({ip}))}catch(err){setError(err.message||'Could not load IP information')}finally{setLoading(false)}}
function renderForm(){return <form onSubmit={run} noValidate className="grid-form" data-optional-submit="true"><label>IP address (optional)<input value={ip} onChange={e=>{setIp(e.target.value);setIpError('')}} inputMode="decimal" pattern="(?:[0-9]{1,3}[.]){3}[0-9]{1,3}" title="Enter an IPv4 address such as 8.8.8.8, or leave it empty to detect yours" aria-invalid={ipError?'true':'false'} aria-describedby="ip-error" placeholder="8.8.8.8" data-api-ip="true"/>{ipError&&<span id="ip-error" role="alert">{ipError}</span>}</label><button className="primary" disabled={loading} data-api-submit="true">Look up</button></form>}
function renderResult(){return <section aria-live="polite" data-api-result="true"><h2>{data.ip}</h2><dl><dt>City</dt><dd>{data.city||'Unknown'}</dd><dt>Region</dt><dd>{data.region||'Unknown'}</dd><dt>Country</dt><dd>{data.country_name||'Unknown'}</dd><dt>Network</dt><dd>{data.org||'Unknown'}</dd></dl></section>}`);
