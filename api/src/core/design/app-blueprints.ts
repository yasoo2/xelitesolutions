...
export function derivedColumns(requestRaw: string): DerivedField[] | null {
    const request = String(requestRaw || '');
    const recording = /(أسجل|اسجل|سجّل|أضيف|اضيف|أدخل|ادخل|أدوّن|ادون|فيها|فيه|الحقول|تحتوي على|يحتوي على|\b(?:record|track|log)\b|\b(?:columns?|fields?)\s*(?::|are\b|include\b|for\b))/iu;
    ...
    return parts.map(label => {
        ...
        seen.set(role, n);
        return { label, key: `${role}${n}`, type, role };
    });
}
