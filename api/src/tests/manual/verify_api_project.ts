...
async function main() {
    ...
    const bad = await fetch(`http://127.0.0.1:${p1}/api/dishes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ details: 'no name' }) });
    ...
    const created = await fetch(`http://127.0.0.1:${p1}/api/dishes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'طبق مؤقت', price: '10' }) }).then(J);
    const put = await fetch(`http://127.0.0.1:${p1}/api/dishes/${created.item.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ price: '15 ر.س' }) }).then(J);
    check('PUT updates the row it names', put.ok === true && put.item.price === '15 ر.س' && put.item.name === 'طبق مؤقت');
    const del = await fetch(`http://127.0.0.1:${p1}/api/dishes/${created.item.id}`, { method: 'DELETE' });
    ...
    const made = await fetch(`http://127.0.0.1:${p2}/api/dishes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'صف عبر JSON', price: '9' }) }).then(J);
