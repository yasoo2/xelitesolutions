    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            try {
                ...
                // Delay fit to ensure container has size and terminal is ready
                const safeFit = () => {
                    try {
                        if (containerRef.current && containerRef.current.clientWidth > 0 &&
                            termRef.current && (termRef.current as any)._core) {
                            fitAddonRef.current?.fit();
                        }
                    } catch (e) {
                        console.debug('[Terminal] Safe fit failed (expected during init):', e);
                    }
                };

                // Delay open until container is ready
                const openTerminal = () => {
                    if (!containerRef.current) return;
