    ...
    return (
        <div className="flex h-full bg-[var(--bg-primary)]">
            ...
            <div className="w-1/3 border-r border-[var(--border-color)] flex flex-col">
                <div className="p-3 border-b border-[var(--border-color)] font-semibold text-sm flex items-center gap-2">
                    <ShieldAlert size={16} className="text-red-500" /> Detected Issues
                </div>
                <div className="flex-1 overflow-auto">
                    {errors.map(err => (
                        <div 
                            ...
                            className={`p-3 border-b border-[var(--border-color)] cursor-pointer hover:bg-[var(--bg-hover)] ${selectedError === err.id ? 'bg-[var(--bg-active)]' : ''}`}
                        >
                            <div className="text-xs text-[var(--text-muted)] mb-1">{new Date(err.timestamp).toLocaleTimeString()}</div>
                            <div className="text-sm font-medium text-red-400 line-clamp-2">{err.message}</div>
                ...
                {currentError ? (
                    <>
                        <div className="p-4 border-b border-[var(--border-color)] bg-[var(--bg-primary)]">
                            <h3 className="font-bold text-red-500 mb-2">{currentError.message}</h3>
                            <pre className="text-xs bg-[var(--bg-tertiary)] p-2 rounded overflow-auto max-h-32 text-[var(--text-secondary)] font-mono">
                                {currentError.stack}
                            </pre>
                            ...
                            {!diagnosis && (
                                <button 
                                    ...
                                    disabled={diagnosing}
                                    className="mt-4 btn-primary w-full flex justify-center items-center gap-2"
                                >
                                    {diagnosing ? <Loader2 className="spin" /> : <Play size={16} />}
                                    Run AI Diagnosis
                                </button>
                            )}
                        </div>
                                ...
                                <div className="card p-4 border-l-4 border-l-green-500">
                                    ...
                                    {diagnosis.isAutoFixable ? (
                                        <>
                                            ...
                                            <pre className="bg-[#1e1e1e] p-3 rounded text-xs font-mono text-green-400 overflow-auto max-h-60">
                                                {diagnosis.suggestedFix}
                                            </pre>
                                            <button 
                                                onClick={applyFix}
                                                disabled={fixing}
                                                className="mt-4 w-full py-2 bg-green-600 hover:bg-green-700 text-white rounded font-medium text-sm flex justify-center items-center gap-2"
                                            >
                                                {fixing ? <Loader2 className="spin" /> : <CheckCircle2 size={16} />}
                                                Apply Auto-Fix
                                            </button>
                                        </>
                                    ) : (
