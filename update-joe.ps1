...
if ($isNetworkFailure) {
    ...
    # ============================================================
    #  وما حُفِظ يُعاد — وإلّا فالحفظ محوٌ مؤجَّل.
    #
    #  الأسطر التالية تنقل التعديلات غير الملتزَمة إلى stash قبل
    #  المحو، وهذا صواب: لا يضيع شيء. لكنّها كانت تقف هناك، فتعود
    #  الشجرة إلى حالة GitHub **بصمت** وتُترك الاستعادة لمن يقرأ
    #  سطراً في نافذةٍ سوداء.
    #
    #  وقع ذلك في 04:01:26 وأنا في منتصف تسعة إصلاحاتٍ مقيسة: اختفت
    #  أربعة عشر ملفّ حارسٍ وستّة تعديلاتٍ في المحرّك، ولم يظهر شيء
    #  إلّا حين فشل اختبارٌ كان أخضر قبل دقيقة. الشجرة لا تشكو،
    #  والمخزن لا يُنادي.
    #
    #  فالآن: يُحاول الإرجاع في آخر التحديث. إن رجع نظيفاً فلا شيء
    #  ضاع ولا شيء يُطلب من أحد. وإن تعارض مع ما نزل من GitHub —
    #  وهذا هو الحال الوحيد الذي يستحق انتباهك — يُقال بالاسم
    #  ويبقى المخزن مكانه. الحفظ يبقى، والصمت يزول.
    # ============================================================
    $stashedName = $null
    $dirty = (git status --porcelain)
    if ($dirty) {
        $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
        $stashedName = "joe-backup-$stamp"
        Say "    لديك تعديلات محلية — سأحفظها في stash باسم $stashedName وأُعيدها بعد التحديث" Yellow
        git stash push -u -m $stashedName | Out-Null
    }
    ...
    if ($branch -and $branch -ne 'main' -and $branch -ne 'HEAD') {
        ...
        # ============================================================
        #  ⛔ والتوقّف يُعيد ما خبّأه — وإلّا فهو ليس توقّفاً.
        #
        #  التعليق فوق يقول «فالتوقّف يشمل إعادة الحال كما كانت»، وكان
        #  الكود يُلغي الدمج ثمّ يخرج **تاركاً الـstash في المخزن**،
        #  فتصير شجرة العمل نظيفةً وقد ضاع من صاحبها كلّ ما لم يلتزمه.
        #
        #  وقع هذا مرّتين على جهاز المالك في يومٍ واحد — 04:01 و13:40 —
        #  وفي المرّتين كانت الرسالة «توقّفتُ قبل أن أمسّ شيئاً» بينما
        #  الشيء قد مُسّ فعلاً. **قاعدةٌ مكتوبةٌ في تعليقٍ ولا شيء يُلزمها.**
        # ============================================================
        if ($stashedName) {
            $back = (git stash list | Select-String -SimpleMatch $stashedName | Select-Object -First 1)
            if ($back) {
                $backRef = ($back.Line -split ':')[0]
                git stash pop $backRef 2>$null | Out-Null
                if ($LASTEXITCODE -eq 0) {
                    Say "    (وأعدتُ تعديلاتك غير الملتزَمة إلى مكانها — لم يُمَسّ شيء.)" Green
                } else {
                    Say "    [!] تعديلاتك غير الملتزَمة ما زالت محفوظةً باسم: $stashedName" Yellow
                    Say "        لاستعادتها:  git stash pop $backRef" White
                }
            }
        }
        Say ""
...
# ============================================================
#  وإرجاع ما حُفِظ — انظر التعليق عند git stash push أعلاه.
#
#  `apply` لا `pop`: النسخة تبقى في المخزن حتى بعد نجاح الإرجاع،
#  فإن كان في الإرجاع خطأٌ لم ينتبه له أحد، فالأصل ما زال هناك.
#  و«لا تضع بيضك كلّه في شجرة عمل».
# ============================================================
if ($stashedName) {
    $entry = (git stash list | Select-String -SimpleMatch $stashedName | Select-Object -First 1)
    if ($entry) {
        $ref = ($entry.Line -split ':')[0]
        git stash apply $ref 2>$null | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Say "    وأعدتُ تعديلاتك المحفوظة ($stashedName) إلى مكانها." Green
            Say "    (النسخة باقية في المخزن أيضاً — لم أحذفها.)" DarkGray
        } else {
            # الحال الوحيد الذي يستحق انتباهه: تعديلاته تصادم ما نزل.
            git checkout -- . 2>$null | Out-Null
            Say ""
            Say "[!] تعديلاتك تتعارض مع ما نزل من GitHub، فلم أُرجِعها تلقائياً." Yellow
            Say "    وهي محفوظة كاملةً باسم: $stashedName" Yellow
            Say "    الخطوة الواحدة لاستعادتها ومعالجة التعارض:" Green
            Say ""
            Say "        git stash apply $ref" White
            Say ""
        }
    }
}
