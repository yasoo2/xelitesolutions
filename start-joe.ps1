...
#  ⛔ لماذا صار المتصفّح مرئيّاً وبملفٍّ منفصل — 2026-08-28
#
#  المالك أمضى الليلة يقول «لا أرى شيئاً» ولم يكن مخطئاً. والسببان هنا:
#
#    BROWSER_HEADED=0            → متصفّح الفحص بلا نافذة أصلاً. كلّ ضغطةٍ
#                                  حقيقيّة، وعلى شاشةٍ لا وجود لها.
#    USE_USER_BROWSER_PROFILE=1  → وحين جُعل مرئيّاً حاول فتح ملفّ كروم
#                                  الشخصيّ، وكروم يرفض فتحه مرّتين، فانهار:
#
#      [BrowserManager] Persistent profile unavailable
#        (launchPersistentContext: … browser has been closed)
#      self-QA: skipped (browser failed (page.goto: … browser has been closed))
#
#  فإمّا مخفيّاً يعمل، وإمّا مرئيّاً ينهار — ولا ثالث. الاثنان معاً هما الحلّ:
#  نافذةٌ حقيقيّةٌ بملفٍّ خاصٍّ بجو لا يتصادم مع كروم المالك.
#
#  ⛔ وثمن الملفّ المنفصل صريح: تسجيلات الدخول المحفوظة في كروم المالك لا
#  تصل جو في هذه الجولة. مقبولٌ لأنّ البديل متصفّحٌ ينهار — ومن أراد
#  الملفّ الشخصيّ فليغلق كروم أوّلاً.
...
#  كروم المالك الحقيقيّ، لا Chromium المرفق — «أريد تشغيل الكروم الخاص فيي».
#  مع ملفٍّ منفصل (USE_USER_BROWSER_PROFILE=0) فلا يتصادم مع نافذته المفتوحة.
$env:USE_SYSTEM_CHROME = "1"
$env:BROWSER_PERSISTENT_PROFILE = "0"
#  ⛔ كان هذا السطر يعيد القيمة إلى "1" بعد ضبطها إلى "0" في الأعلى — كاتبان
#  لنفس الإعداد، والأخير يفوز، فيُلغى الإصلاح بصمت. القيمة تُضبط مرّة واحدة
#  فقط، فوق، مع سبب ضبطها.
...
while ($true) {
    ...
    # ============================================================
    # ============================================================
    #  ولا يُطلق النار قبل أن يسأل من الواقف هناك.
    #
    #  قيس على جهاز المالك: مشغّلان يعملان معاً — أحدهما منذ أكثر
    #  من يوم — ومعرّف جو يتغيّر كلّ دقيقة: 09:43 → 09:45 → 10:19 → 10:20.
    #  كلّ مشغّل يحرّر المنفذ قبل أن يستمع، فيقتل جو الآخر، ثمّ
    #  يُعيد التشغيل — فيموت البناء الجاري في منتصفه ويتوقّف السجلّ
    #  حيث كان. ولا جولة حيّة انتهت في ذلك اليوم كلّه.
    #
    #  والحارس كان يقتل من يحمل المنفذ بلا أن يسأل من هو. وجو سليمٌ
    #  يردّ بـ200 ليس محتلّاً — هو المطلوب نفسه، يعمل أصلاً.
    #  فإن أجاب بصحّة، تنسحب هذه النسخة بدل أن تقتله.
    #
    #  ولا يُعطّل هذا التحديث: update-joe.ps1 يوقف جو قبل أن يستدعي
    #  هذا السكربت، فلا يبقى من يردّ بصحّة، وتمضي النسخة الجديدة.
    #  وعمليّةٌ معلّقة لا تردّ تُقتل كما كانت.
    # ============================================================
    $freed = 0
    try {
        ...
        if ($holders.Count -gt 0) {
            $healthy = $false
            try {
                $r = Invoke-WebRequest -Uri 'http://127.0.0.1:5002/api/health' -TimeoutSec 3 -UseBasicParsing
                $healthy = ($r.StatusCode -eq 200)
            } catch { $healthy = $false }
            if ($healthy) {
                Say "    [port] جو يعمل على 5002 ويردّ بصحّة — لا أقتله، وهذه النسخة تنسحب." Green
                Pop-Location
                exit 0
            }
        }
